...
def main():
    ...
    if len(error_doi_preprint) > 0:
        ...
        print("Please either correct the entry or add a manual metadata file in ", manual_json_dir)
...
def get_chemRXiv(doi):
    ...
    json_i["published"] = {"date-parts": [json_i["publishedDate"].split("T")[0].split("-")]}
    ...
    for i, author in enumerate(authors):
        ...
        for j, name in enumerate(first):
            if len(name) == 1:
                first[j] += "."
        first = " ".join(first)
        authors[i] = {"firstName": first, 
                      "lastName":fullName.split(" ")[-1]}

    dict_return = {"title":title, 
                   "published":{"date-parts":[[year, month, day]]},
                   "authors":authors, "URL":url, "container-title": prefix_full, 
...
def get_manual_json(doi):
    ...
    """
    return_json = None
    fileName = manual_json_dir + "/" + doi.replace("/", "-").replace(":","-").strip() + ".json"
    try:
...
def get_author_str(json_i):
    ...
    try:
        ...
        for j, author_json in enumerate(json_i['author']):
            ...
            author_str += transform_given(author_json['given']) + " " + author_json['family']
    except KeyError:
        ...
        for j, author_json in enumerate(json_i['authors']):
            ...
            author_str += transform_given(author_json['firstName']) + " " + author_json['lastName']
    return author_str + ","
